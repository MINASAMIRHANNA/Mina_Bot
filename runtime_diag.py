...
"""runtime_diag.py — Runtime diagnostics (Safe / Best-effort)

What it does:
- Confirms a single DB file path (cfg.DB_FILE + DatabaseManager.db_file)
- Prints key runtime settings (DB-backed)
- SQLite integrity_check + required tables + row counts
- Dashboard reachability (/healthz, /api/healthz, /api/settings, /api/signals)
- /publish smoke test (optionally token protected)

Usage:
  python tools/runtime_diag.py

Tip:
- Start Dashboard first, then bot + monitor, then run this script.
"""
...
def main() -> int:
    ...
    try:
        ...
        p("[DIAG] db_pin_state", f"<unavailable: {_e}>")
    ...
    # DB manager view of the path + settings sample
    try:
    ...
    try:
        ...
        try:
            ...
            try:
                cap = None
                try:
                    cap = int(db.get_setting('max_concurrent_trades') or 0) if db is not None else 0
                except Exception:
